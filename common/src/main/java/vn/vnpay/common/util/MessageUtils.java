...
public class MessageUtils {
    ...
    static {
        try {
            packager = new GenericPackager("configs/netty-server/iso/iso8583JPosXml.xml");
        } catch (ISOException e) {
            logger.debug("ISOException : {}", e);
        }
    }

    public static byte[] pack(String isoMessage) throws ISOException {
        byte[] packedMessage = CommonUtils.convertStringToBytes(isoMessage);
        return packedMessage;
    }

    public static byte[] pack(TransactionMessageWrap transactionMessageWrap) throws ISOException {
    ...
    public static TransactionMessage unpack(byte[] packedMessage) throws ISOException {
